...
- (void)testAddingSourcesTwice {
    ...
    XCTAssertThrowsSpecificNamed([style addSource:geoJSONSource], NSException, NSInternalInconsistencyException);

    MGLRasterSource *rasterSource = [[MGLRasterSource alloc] initWithIdentifier:@"rasterSource" URL:[NSURL new] tileSize:42];
    [style addSource:rasterSource];
    XCTAssertThrowsSpecificNamed([style addSource:rasterSource], NSException, NSInternalInconsistencyException);

    MGLVectorSource *vectorSource = [[MGLVectorSource alloc] initWithIdentifier:@"vectorSource" URL:[NSURL new]];
    [style addSource:vectorSource];
    XCTAssertThrowsSpecificNamed([style addSource:vectorSource], NSException, NSInternalInconsistencyException);
}
...
- (void)testAddingLayersTwice {
    ...
    XCTAssertThrowsSpecificNamed([style addLayer:backgroundLayer], NSException, NSInternalInconsistencyException);

    MGLCircleStyleLayer *circleLayer = [[MGLCircleStyleLayer alloc] initWithIdentifier:@"circleLayer" source:source];
    [style addLayer:circleLayer];
    XCTAssertThrowsSpecificNamed([style addLayer:circleLayer], NSException, NSInternalInconsistencyException);

    MGLFillStyleLayer *fillLayer = [[MGLFillStyleLayer alloc] initWithIdentifier:@"fillLayer" source:source];
    [style addLayer:fillLayer];
    XCTAssertThrowsSpecificNamed([style addLayer:fillLayer], NSException, NSInternalInconsistencyException);

    MGLLineStyleLayer *lineLayer = [[MGLLineStyleLayer alloc] initWithIdentifier:@"lineLayer" source:source];
    [style addLayer:lineLayer];
    XCTAssertThrowsSpecificNamed([style addLayer:lineLayer], NSException, NSInternalInconsistencyException);

    MGLRasterStyleLayer *rasterLayer = [[MGLRasterStyleLayer alloc] initWithIdentifier:@"rasterLayer" source:source];
    [style addLayer:rasterLayer];
    XCTAssertThrowsSpecificNamed([style addLayer:rasterLayer], NSException, NSInternalInconsistencyException);

    MGLSymbolStyleLayer *symbolLayer = [[MGLSymbolStyleLayer alloc] initWithIdentifier:@"symbolLayer" source:source];
    [style addLayer:symbolLayer];
    XCTAssertThrowsSpecificNamed([style addLayer:symbolLayer], NSException, NSInternalInconsistencyException);
}
